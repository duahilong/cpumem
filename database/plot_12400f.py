# -*- coding: utf-8 -*-
"""Intel i5-12400F 全部价格走势折线图（按日期排序）。

- 合并两个 product_key（i5-12400f 旧键 / intel-i5-12400f 新键）
- 按 price_type 分线：散片 / 原盒 / 默认
- 排除模拟数据（source_image='mock_01.png'）
输出：charts/i5-12400f_all.png
"""
import sqlite3
import os
...
print("=" * 78)
print("【Intel i5-12400F 全部真实价格记录（按日期排序）】")
print("-" * 78)
...
ax.set_title("Intel i5-12400F 价格走势（全部价格，按日期排序）", fontsize=14)
ax.set_xlabel("日期")
ax.set_ylabel("价格 (元)")
ax.grid(True, alpha=0.3)
ax.legend()
fig.tight_layout()
path = os.path.join(OUT_DIR, "i5-12400f_all.png")
